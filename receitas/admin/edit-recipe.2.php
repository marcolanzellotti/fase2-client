...
if (issetPostFields(['title', 'content', 'cat'])) {
    ...
    $cat = E(intval($_POST['cat']));


    $qUpdateRecipe = Q("UPDATE recipes_bkp SET cat='$cat', content='$content', title='$title' WHERE id=$id");
    if ($qUpdateRecipe) {
        // Updated recipe
    }
    if (isset($_FILES['cover'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], "../assets/img/covers/$name.jpg")) {
            $date = date("d/m/Y");
            $qUpdateCover = Q("UPDATE recipes_bkp SET cover='$cover' WHERE id=$id");
            if ($qUpdateCover) {
                // Updated cover
            }
        }
...
$qRecipe = Q("SELECT * FROM recipes_bkp WHERE id=$id");
